...
for my $i (0..$#filenames) {
  ...
  if (index($inputfiles[$i],'s_perp') != -1 || index($inputfiles[$i],'s_par') != -1 ) {
...
for my $i (0..$#filenames) {
  my $syscall;
  push @gnuplotargs, qq(FILE='$tempfiles[$i]'; OUTPUT='$outputfiles[$i]'; PLOTTITLE = '$plottitle'; LINETITLE = '$linetitles[$i]'; PALETTE = '$palette');
